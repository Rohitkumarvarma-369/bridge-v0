...
app = FastAPI(title="XAgent Route API", description="API for XAgent powered by Ollama with route-based team structure")
...
async def ask(
    ...
    try:
        ...
        xagent_team = Team(
            name="XAgent Team",
            mode="route",  # Using route mode instead of coordinate
            model=ollama_model,
            members=[souledstore_agent],
            instructions=[
                "You are XAgent, a helpful assistant that routes user requests to the most appropriate team member.",
                "CRITICALLY IMPORTANT: Return team member responses as RAW STRINGS EXACTLY as received, including any markers.",
                ...
                "Do not add any additional text, commentary, or formatting to team member responses.",
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run(app, host="0.0.0.0", port=8000)  # Use a different port to avoid conflict with the coordinate version
